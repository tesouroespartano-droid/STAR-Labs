...
public final class RetryAndFollowUpInterceptor implements Interceptor {
    ...
    public Response intercept(Interceptor.Chain chain) throws IOException {
        ...
        while (!this.canceled) {
            try {
                try {
                    try {
                        Response responseProceed = realInterceptorChain.proceed(request2, streamAllocation2, null, null);
                        if (response != null) {
                            responseProceed = responseProceed.newBuilder().priorResponse(response.newBuilder().body(null).build()).build();
                        }
                        try {
                            Request requestFollowUpRequest = followUpRequest(responseProceed, streamAllocation2.route());
                ...
                if (!recover(e3, streamAllocation2, !(e3 instanceof ConnectionShutdownException), request2)) {
                    throw e3;
                }
            }
        }
    ...
    private Request followUpRequest(Response response, Route route) throws IOException {
        ...
        if (iCode == 307 || iCode == 308) {
            if (!strMethod.equals(Wb.a) && !strMethod.equals("HEAD")) {
                return null;
            }
        } else {
            if (iCode == 401) {
        ...
        if (HttpMethod.permitsRequestBody(strMethod)) {
            ...
            if (HttpMethod.redirectsToGet(strMethod)) {
                builderNewBuilder.method(Wb.a, null);
            } else {
