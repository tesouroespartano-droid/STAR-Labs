...
public abstract class AbstractFuture<V> implements ListenableFuture<V> {
    ...
    public String toString() {
        ...
        if (isCancelled()) {
            ...
            if (strPendingToString != null && !strPendingToString.isEmpty()) {
                sbAppend.append("PENDING, info=[").append(strPendingToString).append(C0198d4.j.e);
            } else if (isDone()) {
                addDoneString(sbAppend);
            } else {
        ...
        return sbAppend.append(C0198d4.j.e).toString();
    }
    ...
    protected String pendingToString() {
        Object obj = this.value;
        if (obj instanceof SetFuture) {
            return "setFuture=[" + userObjectToString(((SetFuture) obj).future) + C0198d4.j.e;
        }
    ...
    private void addDoneString(StringBuilder builder) {
        try {
            builder.append("SUCCESS, result=[").append(userObjectToString(getUninterruptibly(this))).append(C0198d4.j.e);
        } catch (CancellationException unused) {
            builder.append("CANCELLED");
        } catch (RuntimeException e) {
            builder.append("UNKNOWN, cause=[").append(e.getClass()).append(" thrown from get()]");
        } catch (ExecutionException e2) {
            builder.append("FAILURE, cause=[").append(e2.getCause()).append(C0198d4.j.e);
        }
    }
